...
void Engine::draw() {
	...
	if (map.getPlayer()->getState() == "start") {
		...
		text.setString("coins:" + std::to_string(map.getPlayer()->getCoins()) + " hp: " + std::to_string(map.getPlayer()->getHp()));
		text.setPosition(camera->getView().getCenter().x + 200, camera->getView().getCenter().y - 240);
